...
const Communication = () => {
  ...
  const conversations = [
    { id: 1, type: 'student', name: 'Alex Johnson', lastMessage: 'Thank you for the feedback on my project!', time: '10:30 AM', unread: 0, online: true, avatar: 'AJ' },
    { id: 2, type: 'guardian', name: 'Michael Johnson (Parent)', lastMessage: "Could we schedule a meeting to discuss Alex's progress?", time: '9:15 AM', unread: 2, online: false, avatar: 'MJ' },
    { id: 3, type: 'student', name: 'Sarah Chen', lastMessage: 'I have a question about the assignment deadline', time: 'Yesterday', unread: 1, online: true, avatar: 'SC' }
  ];

  const groupChats = [
    { id: 1, name: 'CS 2024 - Data Structures', members: 45, lastMessage: 'Assignment due date extended to Friday', time: '2:30 PM', unread: 3, avatar: 'DS' },
    { id: 2, name: 'Machine Learning Project Group', members: 12, lastMessage: 'Great work on the presentation everyone!', time: '1:45 PM', unread: 0, avatar: 'ML' },
    { id: 3, name: 'Computer Science Faculty', members: 8, lastMessage: 'Meeting scheduled for tomorrow at 3 PM', time: '11:20 AM', unread: 1, avatar: 'CS' }
  ];

  const messages = [
    { id: 1, sender: 'Alex Johnson', text: 'Hi Dr. Johnson, I wanted to thank you for the detailed feedback on my data structures project.', time: '10:25 AM', isMe: false },
    { id: 2, sender: 'Me', text: "You're welcome, Alex! Your implementation was really well thought out. Keep up the good work!", time: '10:27 AM', isMe: true },
    { id: 3, sender: 'Alex Johnson', text: 'I was wondering if you could help me understand the time complexity analysis better?', time: '10:28 AM', isMe: false },
    { id: 4, sender: 'Me', text: "Of course! Let's schedule a quick meeting this week. Are you free Thursday afternoon?", time: '10:29 AM', isMe: true },
    { id: 5, sender: 'Alex Johnson', text: 'Thank you for the feedback on my project!', time: '10:30 AM', isMe: false }
  ];
    ...
    <div className="h-[calc(100vh-12rem)] flex bg-white dark:bg-black rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div className="w-80 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'individual' && conversations.map((conv) => (
            <div
              key={conv.id}
          ...
          {activeTab === 'groups' && groupChats.map((group) => (
            <div
              key={group.id}
              className="p-4 border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800"
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium text-gray-900 dark:text-gray-100 truncate">{group.name}</h3>
                    <span className="text-xs text-gray-500 dark:text-gray-400">{group.time}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 truncate mt-1">{group.lastMessage}</p>
                </div>
              </div>
            </div>
          ))}
              ...
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-xs lg:max-w-md ${message.isMe ? 'order-2' : 'order-1'}`}>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 px-2">{message.time}</p>
                  </div>
